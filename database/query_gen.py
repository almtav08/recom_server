...
class QueryGenerator:
    ...
    def select_users_by_pass(self, is_pass: bool = True) -> list[User]:
        statement = select(User).where(User.is_pass == is_pass)
        users = self.session.exec(statement).all()
        return users

    def select_users_by_resource_and_passed(self, res_id: int, is_pass: bool = True):
        statement = (
            select(User)
            .join(Interaction)
            .where(Interaction.resource_id == res_id, Interaction.passed == is_pass)
        )
        users = self.session.exec(statement).all()
        return users
    ...
    def select_interactions_by_user_and_resource(
