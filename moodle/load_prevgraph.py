...
GRAPH_PATH = "./moodle/data/prev_graph.json"
...
def _read_inputs() -> Dict[int, List[int]]:
    """Read user-entered prerequisite relations keyed by recid (or id if toggled)."""
...
def _validate_graph(graph: Dict[int, List[int]]):
    ...
    for src, targets in graph.items():
        if len(targets) != len(set(targets)):
            errors.append(f"Duplicate {key_label}s in prereqs of {src}")
...
def _load_existing_graph():
    if not os.path.exists(GRAPH_PATH):
        _log("prev_graph.json does not exist, starting empty.")
        return
    try:
        ...
        _log("Previous graph loaded.", (120, 220, 120, 255))
...
def build_ui():
    with dpg.window(
        tag="main_window", label="Moodle Prerequisites", width=900, height=600
    ):
        dpg.add_text("Define prerequisite relations (IDs separated by commas)")
        ...
        with dpg.child_window(border=True, autosize_x=True, autosize_y=True):
            with dpg.table(
                ...
                dpg.add_table_column(label="Prereqs (RecIDs)")
                for m in course_modules:
                    with dpg.table_row():
...
def main():
    ...
    dpg.create_context()
    dpg.create_viewport(title="Moodle Prerequisites", width=930, height=640)
    build_ui()
    dpg.setup_dearpygui()
    dpg.show_viewport()
    _load_existing_graph()
